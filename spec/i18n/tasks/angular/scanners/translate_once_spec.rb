...
RSpec.describe I18n::Tasks::Angular::Scanners::TranslateOnce do
  subject(:translate_once) { described_class.new }

  describe '#pattern' do
    context 'double quotes' do
      let(:text) { '.all-caps-label(translate-once="activerecord.attributes.proposal_term.deal_type")' }
      it { expect(text.scan(translate_once.pattern).flatten.size).to eq(1) }
      it { expect(text.scan(translate_once.pattern).flatten.first).to eq('activerecord.attributes.proposal_term.deal_type') }
    end
...
TEXT
      ...
      it { expect(text.scan(translate_once.pattern).flatten.size).to eq(0) }
    end
